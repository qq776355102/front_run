...
async function createWeb3(){
    try {
         web31 = new Web3(new Web3.providers.HttpProvider(HTTP_PROVIDER_LINK));
        // web31 = new Web3(new Web3.providers.HttpProvider("https://bsc-dataseed1.defibit.io"));
        web3 = new Web3(new Web3.providers.HttpProvider("https://bsc-dataseed1.defibit.io"));
        web32 = new Web3(new Web3.providers.HttpProvider("https://bsc-dataseed.binance.org"));
        web33 = new Web3(new Web3.providers.HttpProvider("https://bsc-dataseed1.ninicoin.io"));
...
try {   
        ...
        nonce = await web3.eth.getTransactionCount(user_wallet.address);
        ...
        // await approve(gas_price_info.high, outputtoken, out_token_address, user_wallet); //正式环境打开
        ...
        subscription = web3Ws.eth.subscribe('pendingTransactions', function (error, result) {
            ...
            try {
                ...
                if(i== 0){
                    ...
                    transaction = await web31.eth.getTransaction(transactionHash);
                }
                else if(i== 2){
                    transaction = await web32.eth.getTransaction(transactionHash);
                }
                else if(i== 3){
                    transaction = await web33.eth.getTransaction(transactionHash);
                }
                console.log(i);
                ...
                if (transaction != null && transaction['to'] != null && transaction['to'].toLowerCase() == PANCAKE_ROUTER_ADDRESS.toLowerCase() && transaction.value > amount*(10**18))
                {

                    if(check(transaction.value,amount*1.2*(10**18),pool_info.input_volumn,pool_info.output_volumn)){
...
async function handleTransaction(transaction, out_token_address, user_wallet, amount, level) {
    ...
    if (await triggersFrontRun(transaction, out_token_address, amount, level)) {
        ...
        //buy gasprice
        let newGasPrice = gasPrice + 20*ONE_GWEI;
        ...
        var outputtoken = getAmountOut(estimatedInput, pool_info.input_volumn, pool_info.output_volumn);
        ...
        // var outputeth = getAmountOut(outputtoken, pool_info.output_volumn, pool_info.input_volumn);
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10**9 > 10 && parseInt(transaction['gasPrice']) / 10**9 <= 70){
        attack_started = true;
        // return true
    }else{
        return false;
    ...
    if(method == 'swapExactETHForTokens')
    {
        ...
        await updatePoolInfo();
        let log_str = "swapExactETHForTokens-1"+'\t'+"Attack ETH Volumn : Pool Eth Volumn" + '\t\t' +(pool_info.attack_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info.input_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
        ...
        await updatePoolInfo();
        let log_str = "swapETHForExactTokens-2"+'\t'+"Attack ETH Volumn : Pool Eth Volumn" + '\t\t' +(pool_info.attack_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info.input_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
    ...
    else if(method == 'swapExactTokensForTokens')
    {
        ...
        await updatePoolInfo();
        let log_str = "swapExactTokensForTokens-3"+'\t'+"Attack ETH Volumn : Pool Eth Volumn" + '\t\t' +(pool_info.attack_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info.input_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
        console.log(log_str.green);
    ...
    else if(method == 'swapTokensForExactTokens')
    {
        ...
        await updatePoolInfo();
        let log_str = "swapTokensForExactTokens-4"+'\t'+"Attack ETH Volumn : Pool Eth Volumn" + '\t\t' +(pool_info.attack_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info.input_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
        console.log(log_str);
...
async function swap(gasPrice, gasLimit, outputtoken, outputeth, trade, out_token_address, user_wallet, transaction) {
    ...
    await web3.eth.getBlock('latest', (error, block) => {
        deadline = block.timestamp + 120; // transaction expires in 300 seconds (5 minutes)
    });

    deadline = web3.utils.toHex(deadline);
    console.log("deadline  :"+deadline+"    trade :  "+trade);
    ...
    if(trade == 0) { //buy
      ...
      //  nonce = await web3.eth.getTransactionCount(from.address);
        ...
        console.log('Get_Min_Amount '.yellow, (outputeth/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol+"  \t"+(outputtoken/(10**out_token_info.decimals)).toFixed(3) + ' ' + out_token_info.symbol);
    ...
    console.log('====signed transaction=====', gasLimit, gasPrice,nonce);
    ...
    .on('error', function(error, receipt) { // If the transaction was rejected by the network with a receipt, the second parameter will be the receipt.
        if(trade == 0){
          ...
          //buy_finished = true; //测试专用
        }
        else{
          sell_failed = true;   
          console.log('Attack failed(sell)   '+error);
          console.log('receipt   '+receipt);
          //sell_finished = true;   //测试专用
        }
...
async function getTokenInfo(tokenAddr, token_abi_ask, user_wallet) {
    try{
        ...
        //get token info
        token_contract = new web3.eth.Contract(JSON.parse(token_abi), tokenAddr);
...
main();
    ...
    function getAmountOut(amountIn, reserveIn,reserveOut)  {
        ...
        // var amountInWithFee = amountIn.mul(9975);
        // var numerator = amountInWithFee.mul(reserveOut);
        // var denominator = reserveIn.mul(10000).add(amountInWithFee);
        //  numerator / denominator;
        return (amountIn*9975*reserveOut)/(reserveIn*10000+amountIn*9975);
    ...
    function getAmountIn(amountOut, reserveIn,reserveOut) {
        ...
        return ((reserveIn*amountOut*10000)/((reserveOut-amountOut)*9975));
...
//amountIn：用户投入金额
//AMOUNT：投资金额：
function check(amountIn, AMOUNT, reserveIn,reserveOut){

    
    
    var p = getAmountOut(AMOUNT, reserveIn,reserveOut);
   // console.log("p",p/10**18);
    var reserveOut_ = reserveOut-p;
    var reserveIn_ = reserveIn+AMOUNT*9975/10000;
  //  console.log("reserveIn_  :",reserveIn_);

    var p1 = getAmountOut(amountIn, reserveIn_,reserveOut_);
   // console.log(p1);

    var reserveIn_ = reserveIn_+ amountIn*9975/10000;
    var reserveOut_ = reserveOut_-p1;
    var AMOUNT_ = getAmountIn(p,reserveIn_, reserveOut_);

   // console.log(AMOUNT_/10**18);
    return  AMOUNT_ - AMOUNT > 0.007*10**18
